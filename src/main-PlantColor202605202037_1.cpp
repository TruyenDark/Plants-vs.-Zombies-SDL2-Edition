...
struct Zombies {
    ...
    void tryMoveZombie() {
        if (SDL_GetTicks() >= this->nextMoveTime) {
            this->move();
...
void spawnZombie(Yard& yard) {
    ...
    Zombies zombie = {{(yard.plots[random_row][8].rect.x + yard.plots[random_row][8].rect.w), 0, 175/2, 200}, 0, 100, 16, false, 500, (SDL_GetTicks() + 500), random_row};
                    ...
                    for (int i = 0; i < 5; i++) {
                        for (int j = 0; j < 9; j++) {
                            if (xFinger >= yard.plots[i][j].rect.x && xFinger <= (yard.plots[i][j].rect.x + yard.plots[i][j].rect.w) && yFinger >= yard.plots[i][j].rect.y && yFinger <= (yard.plots[i][j].rect.y + yard.plots[i][j].rect.h) && yard.plots[i][j].hasPlant == false) {
                                ...
                                Plants plant = {{yard.plots[i][j].rect.x, yard.plots[i][j].rect.y, yard.plots[i][j].rect.w, yard.plots[i][j].rect.h}, 100, 1000, yard.selectCard, i};
