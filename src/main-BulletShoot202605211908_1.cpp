...
void spawnZombie(Yard& yard) {
    ...
    Zombies zombie = {{(yard.plots[random_row][8].rect.x + yard.plots[random_row][8].rect.w), 0, 175/2, 200}, 0, 100, 16, false, 500, (SDL_GetTicks() + 500), random_row, 1};
    ...
    while (isRunning) {
                    ...
                    for (int i = 0; i < 5; i++) {
                        for (int j = 0; j < 9; j++) {
                            if (xFinger >= yard.plots[i][j].rect.x && xFinger <= (yard.plots[i][j].rect.x + yard.plots[i][j].rect.w) && yFinger >= yard.plots[i][j].rect.y && yFinger <= (yard.plots[i][j].rect.y + yard.plots[i][j].rect.h) && yard.plots[i][j].hasPlant == false) {
                                ...
                                Plants plant = {{yard.plots[i][j].rect.x, yard.plots[i][j].rect.y, yard.plots[i][j].rect.w, yard.plots[i][j].rect.h}, 100, 1000, yard.selectCard, i, j, false, 1500, (SDL_GetTicks() + 1500)};
        ...
        for (int i = 0; i < yard.plant_list.size(); i++) {
            ...
            if (yard.plant_list[i].shoot == true && SDL_GetTicks() >= yard.plant_list[i].nextShootTime) {
                Bullets bullet = {{yard.plant_list[i].rect.x + yard.plant_list[i].rect.w, yard.plant_list[i].rect.y, 50, 50}, 0, 25, 1, yard.plant_list[i].row};
