...
struct Plants {
    ...
    unsigned int sunProductionTimer = 16000;
...
struct Yard {
    ...
    unsigned int sunSpawnInterval = 25000;
...
void initSeedBar(Yard& yard) {
    ...
    for (int i = 0; i < 11; i++) {
        SeedSlots card = {{vector2.x, vector2.y, 100, 120}, (i + 1), 100, true};
...
void spawnSunflowerSun(Yard& yard) {
    for (int i = 0; i < yard.plant_list.size(); i++) {
        if (yard.plant_list[i].id == 2 && SDL_GetTicks() >= yard.plant_list[i].sunSpawnTimer) {
            Suns sun = {{yard.plant_list[i].rect.x, yard.plant_list[i].rect.y, yard.plant_list[i].rect.w, yard.plant_list[i].rect.h}, 25, false, 0, SDL_GetTicks() + 3000};
...
void spawnSkySun(Yard& yard) {
    if (SDL_GetTicks() >= yard.sunSpawnCooldown) {
        ...
        Suns sun = {{random_x, -120, 120, 120}, 25, true, random_targetY, SDL_GetTicks() + 25000};
        yard.sun_list.push_back(sun);
                    ...
                    for (int i = 0; i < 5; i++) {
                        for (int j = 0; j < 9; j++) {
                            if (xFinger >= yard.plots[i][j].rect.x && xFinger <= (yard.plots[i][j].rect.x + yard.plots[i][j].rect.w) && yFinger >= yard.plots[i][j].rect.y && yFinger <= (yard.plots[i][j].rect.y + yard.plots[i][j].rect.h) && yard.plots[i][j].hasPlant == false) {
                                ...
                                Plants plant = {{yard.plots[i][j].rect.x, yard.plots[i][j].rect.y, yard.plots[i][j].rect.w, yard.plots[i][j].rect.h}, 100, 1000, yard.selectCard, i, j, false, 3000, (SDL_GetTicks() + 3000), 16000, (SDL_GetTicks() + 16000)};
